...
async function scrape(companyList) {
  ...
  if (isLoginPage) {
    await page.type('#username', 'sgrrsaurabh564@gmail.com');
    await page.type('#password', 'Saumya99@');
...
for (const pageUrl of comanyLinkedinUrl) {
  ...
  let data = combinedArrays(companyData, peopleDetail)
  data = combinedArrays(data, companyDetailsAbout)
  console.log(data);
...
fs.createReadStream('company.csv')
  ...
  .on('end', () => {
    ...
    if (companyList.length > 0) {
      ...
      console.error('No valid company names f');
    }
  });